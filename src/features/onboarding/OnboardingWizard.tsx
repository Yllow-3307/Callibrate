    ...
    >
      ...
      {checked && (
        <motion.span
          ...
          style={{ color: 'rgb(var(--color-accent-rgb))', display: 'grid' }}
        >
      ...
      >
          ...
          >
            {step === 1 && (
              <Step
                ...
                subtitle="Ces informations nous aideront à adapter tes futures séances."
                icon="👋"
              >
            ...
            {step === 2 && (
              <Step
                ...
                subtitle="Choisis l'objectif qui te ressemble le plus."
                icon="🎯"
              >
                <div className="choice-list">
                  {['Sèche', 'Prise de muscle', 'Affinement musculaire sans prise de poids', 'Travail de figures de callisthénie'].map((item) => (
                    <motion.label
                      key={item}
                      className="choice"
                      data-checked={data.objectif === item}
                      whileTap={{ scale: 0.98 }}
                      whileHover={{ y: -1 }}
                    >
                      ...
                      {data.objectif === item && (
                        <span style={{ color: 'rgb(var(--color-accent-rgb))' }}>
              ...
              >
                <div className="choice-list">
                  {equipmentChoices.map((item) => (
                    <Choice key={item} value={item} checked={data.equipements.includes(item)} onChange={() => toggleEquipment(item)}>
                      {item}
                    </Choice>
                  ))}
                </div>
                {data.equipements.includes('Équipement personnel') && (
                  <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}>
                    <label>
                      Précise ton équipement personnel
                      <input className="input-base" value={data.detailsPerso} onChange={(e) => set('detailsPerso', e.target.value)} placeholder="Ex. haltères, élastiques…" />
                    </label>
                  </motion.div>
                )}
                <SelectField label="Équipement d'endurance disponible" value={data.endurance} onChange={(v) => set('endurance', v)} options={['Course à pied', 'Vélo', 'Corde à sauter', 'Aucun', 'Autre']} />
              </Step>
            )}
            ...
            {step === 4 && (
              <Step
                ...
                subtitle="Pour faire une place réaliste au sport dans tes journées."
                icon="⏰"
              >
                <div className="grid-2">
            ...
            {step === 5 && (
              <Step
                title="Côté cuisine"
                subtitle="Quelques repères pour préparer la partie nutrition."
                icon="🥗"
              >
                <div className="grid-2">
        ...
        <footer className="wizard-actions">
          ...
          <Button variant={step === 5 ? 'warm' : 'primary'} onClick={step === 5 ? save : next} disabled={saving} isLoading={saving} size="lg">
            {saving ? 'Enregistrement…' : step === 5 ? (
              <>
                <span style={{ fontSize: '1.1em' }}>🔥</span> Enregistrer mon profil
              </>
            ) : 'Suivant'}
            {step < 5 && !saving && (
  ...
  return (
    <div className="step">
      ...
      <motion.div
        ...
        style={{ display: 'grid', gap: 18, marginTop: 6 }}
      >
        {children}
      </motion.div>
    </div>
